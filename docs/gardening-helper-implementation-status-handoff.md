...
Last updated: 2026-07-02 (AI plant ingestion enhancement: photo, group, variety, follow-up questions and refine flow)
...
- Phase 24.5 accuracy tuning (2026-06-14): product ingestion was under-detecting products and omitting spraying rules. Set deterministic `temperature: 0` for all OpenAI extraction calls; rewrote the `ingestProduct` prompt to mandate a spraying/application rule for sprayable plant-protection products with honest-uncertainty warnings and input-language free text; switched `ingestProduct` to strict `json_schema` Structured Outputs enforcing valid category/unit enums; product_rule is normalized so all-null (non-sprayable) rules are dropped while partial rules (e.g. dilution-only) are kept. Recommended `AI_MODEL` updated to `gpt-5.4` (alternatives gpt-5.4-mini / gpt-5.5) in env.example. Added 3 adapter unit tests (temperature+schema assertion, all-null rule dropped, dilution-only rule kept); AI adapter suite passes 12/12. No API contract, schema, or domain-invariant changes — AI output remains a suggestion, not business truth.
- Phase 24.5 web search for product ingestion (2026-06-14): migrated `ingestProduct` from Chat Completions to the OpenAI Responses API so it can call the hosted `web_search` tool to look up products the model does not already know (official label / SDS). The prompt instructs the model to prefer official/manufacturer/regulatory sources, never invent doses, and cite the sources it used in `warnings`. Web search runs on OpenAI's side (no extra outbound access from this server). Added `AI_WEB_SEARCH` backend-only config (default enabled; set `false` to disable) wired through `createAiAdapter` into a new `OpenAiAdapterOptions.webSearch`. `suggestBedPlan`/`assistProblem` remain on Chat Completions. Shared OpenAI error mapping extracted to `mapError`. Added 2 adapter tests (web_search tool present when enabled, omitted when disabled); backend suite 284 pass / 216 skipped. Still a suggestion-only flow through the existing accept/reject workflow.
- AI plant ingestion enhancement (2026-07-02): extended `POST /api/v1/ai/plant-ingestion` to accept optional `group`, `variety`, `photoDataUrl` (base64 data URL, jpeg/png/webp, size-capped via `problemPhotoMaxBytes`, route-level bodyLimit) and `followUpAnswers`; `plantName` is now optional with a name-OR-photo refinement. The photo is ephemeral AI-only input — forwarded to the provider as a Responses API `input_image` part (works together with web_search + strict json_schema) and never persisted (session `raw_input_text` records only the text fields; inputMode name/image/mixed). The plant ingestion schema/prompt now returns typed `followUpQuestions` mapped to a `followup_questions` suggestion (guidance-only accept path already existed); a given variety focuses the search on that variety (1–2 suggestions). Frontend: `PlantIngestionPage` gained group/variety fields, a single-photo picker with preview reusing new shared `shared/utils/image-compression.ts` (extracted from `ProblemPhotoUploader`), a name-or-photo form validator, a follow-up questions block (yes/no radios + free text) with a refine submit that resends the same request plus answers and the in-memory photo. TestAiAdapter mirrors the variety-focus/follow-up contract. Tests: 577 backend (incl. AI routes DB integration) + 264 frontend pass.
...
Implemented. See `docs/implementation-phases/phase-24/`.

- [x] Step 1 - AI API Services and Feature Scaffold.
- [x] Step 2 - Shared AI Suggestion Card and Review State.
- [x] Step 3 - Product Ingestion Page.
- [x] Step 4 - Bed Planning Page.
- [x] Step 5 - Problem Assist Page.
- [x] Step 6 - Phase 24 Frontend Regression, Boundary, and Error Tests.
- [x] Step 7 - Phase 24 Verification and PR Readiness.
